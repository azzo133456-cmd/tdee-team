...
const EXS = {
  ...
  // 跑步機爬坡：坡度把強度拉得比平路高非常多，記成「走路(快)」會少算一半以上。
  // 依 ACSM 走路耗氧公式 VO2 = 0.1×S + 1.8×S×G + 3.5（S 為 m/min、G 為坡度）：
  //   5km/h 坡 6% → 6.0 MET　　5km/h 坡 12% → 8.5 MET（約等於 8km/h 跑步，但對膝蓋友善）
  "坡度健走(坡5-8%)":6.0,"坡度快走(坡10-15%)":8.5,
...
const STRENGTH_BY_MUSCLE={
  ...
  "背":["硬舉","引體向上","滑輪下拉","寬握滑輪下拉","槓鈴划船","啞鈴單臂划船","坐姿划船","坐姿寬握滑輪划船","T槓划船","直臂下壓","反式划船","山羊挺身"],
  "腿":["深蹲","前蹲舉","哈克深蹲","史密斯深蹲","腿推","腿伸屈","腿後勾","趴姿腿後勾","羅馬尼亞硬舉",
       "保加利亞分腿蹲","弓步蹲","腿外展機","腿內收機","單腳繩索內收","單腳繩索前抬"],
  // 單腳繩索系列＝站在滑輪機前、單腳做四個方向的髖動作。單邊訓練能抓出左右差異，
  // 也是「已經在練後側鏈的人」該往下走的進階方向（見下方 posteriorLine 的建議）。
  "臀":["臀推","髖外展機","臀橋","驢踢","早安運動",
       "繩索側踢","單腳繩索後踢","單腳繩索側抬","單腳繩索硬舉"],
  "肩":["肩推","啞鈴肩推","阿諾肩推","史密斯肩推","側平舉","前平舉","反向飛鳥","上斜反向飛鳥","面拉","直立划船","聳肩"],
...
function renderExRecF(){
  ...
  // 上下半身分開判定，不可合併：影片講的 Q-angle 與前十字韌帶風險是純下半身的論點。
  // 若把划船、面拉也算進「後側鏈達標」，一個只練上背、完全不做髖鉸鏈的人會被判定為
  // 已經做到 —— 而她正是最需要被提醒的人。
  const POSTERIOR_LOWER=["硬舉","早安","山羊挺身","臀推","臀橋","驢踢","側踢","後踢","側抬","髖外展","腿後勾","保加利亞分腿蹲","弓步蹲","登階","分腿蹲","背伸展"];
  const POSTERIOR_UPPER=["引體向上","滑輪下拉","划船","面拉","反向飛鳥","直臂下壓"];
  const doneAny=(list)=>ex28.some(e=>e.kind==="strength"&&list.some(k=>String(e.name).includes(k)));
  const lower=doneAny(POSTERIOR_LOWER), upper=doneAny(POSTERIOR_UPPER);
  const lowerLine = lower
    ? `<b>下半身後側鏈</b>你已經有在練 👍 女性骨盆較寬、Q-angle 較大，容易變成股四頭主導，後側練得夠能降低膝與前十字韌帶的風險。`+
      `接下來往<b>單邊動作</b>（單腳羅馬尼亞硬舉、保加利亞分腿蹲）和漸進加重發展。`
    : `⚠️ 課表缺<b>下半身後側鏈</b>：髖鉸鏈類（硬舉、羅馬尼亞硬舉、早安運動）＋臀腿（臀推、腿後勾）。`+
      `深蹲與腿推是膝主導、練不到這裡。女性骨盆較寬、Q-angle 較大，後側不足會讓膝與前十字韌帶承擔更多。`;
  const upperLine = upper
    ? `<b>上半身後側鏈</b>也有覆蓋到，姿勢與肩關節健康顧得不錯。`
    : `<b>上半身後側鏈</b>目前是空的：加上垂直拉（引體向上、寬握滑輪下拉）與水平拉（坐姿寬握滑輪划船），`+
      `再配面拉／上斜反向飛鳥練後三角與中下斜方，久坐造成的圓肩駝背才拉得回來。`;
  const posteriorLine = `<br>${lowerLine}<br>${upperLine}`;
